...
const UserList = ({
  ...
  const changeHandler = (checked, id) => {
    checked ? setCheckedButtons([...checkedButtons, id]) : setCheckedButtons(checkedButtons.filter((el) => el !== id))
  };
  /** 모두 선택해주는 체크박스 */
  const allCheckBox = (e) => {
    if (!isChecked) {
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{width: '10%', backgroundColor: '#F6F7F9' }}>번호</TableCell>
                <TableCell sx={{ backgroundColor: '#F6F7F9' }}>ID</TableCell>
                <TableCell sx={{ backgroundColor: '#F6F7F9' }}>이름</TableCell>
